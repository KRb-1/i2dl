...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            # The first layer: Input dimension (28*28) -> The first hidden layer
            nn.Linear(self.input_size, self.hparams["n_hidden"]),
            nn.BatchNorm1d(self.hparams["n_hidden"]),
            nn.LeakyReLU(0.2),
            
            # The second layer: First hidden layer -> Second hidden layer
            nn.Linear(self.hparams["n_hidden"], self.hparams["n_hidden_2"]),
            nn.BatchNorm1d(self.hparams["n_hidden_2"]),
            nn.LeakyReLU(0.2),
            
            # The third layer (output layer) -> latent_dim
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(self.hparams["n_hidden_2"], self.hparams["n_hidden"]),
            nn.BatchNorm1d(self.hparams["n_hidden"]),
            nn.LeakyReLU(0.2),
...
class Autoencoder(nn.Module):
    ...
    def training_step(self, batch, loss_func):
        ...
        reconstructed_images = self.forward(flattened_images)
        loss = loss_func(reconstructed_images, flattened_images)
...
class Classifier(nn.Module):
    ...
    def __init__(self, hparams, encoder):
        ...
        self.model = nn.Sequential(
            nn.Linear(self.encoder.latent_dim, self.hparams.get("classifier_hidden", 64)),
            nn.BatchNorm1d(self.hparams.get("classifier_hidden", 64)),
            nn.ReLU(),
            nn.Dropout(p=self.hparams.get("dropout", 0.3)),
            nn.Linear(self.hparams.get("classifier_hidden", 64), num_classes)
        )
    ...
    def set_optimizer(self):
        ...
        lr = self.hparams.get("classifier_lr", 1e-3)
