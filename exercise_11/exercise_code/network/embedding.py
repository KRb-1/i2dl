...
def positional_encoding(d_model: int,
    ...
    div_term = torch.exp(
        torch.arange(0, d_model,2,dtype=torch.float32) * (-torch.log(torch.Tensor([10000])) / d_model)
    )

    output = torch.zeros(max_length, d_model)

    output[:, 0::2] = torch.sin(positions * div_term)
    output[:, 1::2] = torch.cos(positions * div_term[:output[:, 1::2].shape[1]])
